...
- **Notarized DMG boundary**: the official DMG is an unsigned transport container. Validate its attached Apple ticket with `xcrun stapler validate`; assess the Developer ID-signed app with `spctl -t exec`. Do not require `spctl -t open` to accept an unsigned DMG.
...
- 2026-07-20: Rewrote pushable history to remove four third-party PDFs and canonicalize Rafal Sikora's legacy Git identity, then published only sanitized `main` to the new `rsitech-ai/LiteratureAtlas` repository. The sanitized baseline contains no matching path, audited blob, or legacy email; the pre-rewrite mirror remains local recovery evidence. GitHub later authored PR #4's merge commit with the account merge identity rather than the repository-configured no-reply email.
...
- 2026-07-21: Released official `v1.0.0` from protected-main SHA `88f7d5e7c373226eb3861277ba9ca6a57f5e8774`. Apple accepted submission `68cc41be-2f44-4650-a731-ec1e5a042f3f`; the stapled published DMG SHA-256 is `f2fc0c5ec19831213e1dcd0ea4d6fe104a890fdaae821a581e5ce5d4af108c20`, and public DMG/checksum/app-SPDX bytes were verified. Release-evidence PR #5 passed all ten hosted checks and rebased reviewed head `d9835891b7276122c45c06d27147966f810d2da9` onto protected `main` as `9d14a50c7eebae61ab2511d21437ee203870f96a`. The legacy personal prerelease/tag and stale Dependabot refs were removed; `s1korrrr/LiteratureAtlas` is private and archived while the local recovery mirror remains.
