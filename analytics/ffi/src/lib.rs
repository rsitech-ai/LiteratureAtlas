...
/// - `query_ptr` must be non-null and point to at least `query_len` readable floats.
/// - `query_len` must equal the index dimension; mismatches are rejected before reading `query_ptr`.
/// - `out_ptr` must be non-null and point to at least `k` writable `AtlasSearchResult` entries.
pub unsafe extern "C" fn atlas_query_index_v2(
...
mod tests {
    ...
    #[test]
    fn hnsw_query_rejects_dimension_mismatch_before_reading_query() {
        let data = [0.0f32, 0.0, 10.0, 10.0];
        let index = unsafe { atlas_build_index(2, 2, data.as_ptr()) };
        assert!(!index.is_null());
        let too_short_query = [9.5f32];
        let mut out = [AtlasSearchResult {
            index: 0,
            distance: 0.0,
        }];
        ...
        let count = unsafe {
            atlas_query_index_v2(
                index,
                too_short_query.as_ptr(),
                too_short_query.len() as u32,
                1,
                out.as_mut_ptr(),
            )
        };
